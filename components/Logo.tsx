  ...
  return (
    <div className={`flex items-center ${gap} ${className}`}>
      <svg width={dim} height={dim} viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        ...
        <line x1="24" y1="22" x2="24" y2="15" stroke={isDark ? "#0d0d0d" : "white"} strokeWidth="1" opacity="0.3" />
      ...
      {showWordmark && (
        <div className="flex flex-col leading-none">
          <span className={`font-black tracking-[0.25em] uppercase ${textSize} ${isDark ? "text-white" : "text-black"}`}>SPM</span>
          <span className={`font-semibold tracking-[0.2em] uppercase ${subSize} ${isDark ? "text-white/50" : "text-black/45"}`}>Taxi · CPAM</span>
        </div>
      )}
    </div>
  );
